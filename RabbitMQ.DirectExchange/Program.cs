            ...
            do
            {
                ...
                using (var connection = factory.CreateConnection())
                {
                    using (var channel = connection.CreateModel())
                    {
                        ...
                        string message = $"Message type [{routingKey}] sent into Direct Exchange [N {counter}]";

                        var body = Encoding.UTF8.GetBytes(message);
                        channel.BasicPublish(exchange: "direct_logs", routingKey: routingKey, basicProperties: null, body: body);
                        Console.WriteLine($"Message type [{routingKey}] sent into Direct Exchange [N {counter++}]");
                    }
                }
